  ...
  * printType (str): 'all', 'books' or 'magazines' restrict the results to either all or one of the preceding types of media only
  ...
  def search(self, query, offset):
    ...
    * urllib2.URLError
    ...
    def parse_google_books_json(site, pos, query, results):
      ...
      try:
        for result in results['items']:
            ...
            if 'subtitle' in result['volumeInfo']:
	            book_dict['title'] += (' ' + result['volumeInfo']['subtitle'])
            book_dict['link'] = result['selfLink']

            if 'description' in result:
                book_dict['summary'] = result['description']
            else:
                book_dict['summary'] = '' # If there's in no description it' up to the app developer to make use of the other data
    ...
    try:    
      ...
      return parse_google_books_json('Google Books', pos, query.search_terms, results)
