  ...
  * explicit (boolean): Do we want to return results marked as including explicit content (not recommended to change this)
  ...
  def search(self, query, offset):
    ...
    * urllib2.URLError
    ...
    def parse_itunes_json(site, query, results):
      ...
      * puppy.model.OpenSearch.Response
      ...
      try:
        for result in results['results']:
            ...
            result_dict['title'] = ''
            result_dict['link'] = ''
            result_dict['summary'] = result['artistName']
            ...
            if 'trackName' in result:
                ...
                # Use censored track name if not wanting explicit content
                if (self.explicit == False) and ('trackCensoredName' in result):
                    result_dict['title'] = result['trackCensoredName']

                result_dict['summary'] = result_dict['title'] + ' by ' + result_dict['summary']
    ...
    try:
      ...
      return parse_itunes_json('iTunes', query.search_terms, results)
    ...
    # Check for a type error for offset or resultsPerPage
	  ...
	  raise SearchEngineError("iTunes", e, note = note)
